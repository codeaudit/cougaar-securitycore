...
public class PSP_Assign extends PSP_BaseAdapter implements PlanServiceProvider, UISubscriber
{
  ...
  public void execute( PrintStream out,HttpInput query_parameters,PlanServiceContext psc,PlanServiceUtilities psu ) throws Exception
  {
	...
	System.out.println("is post");
      ...
      if(MonitoringUtils.debug>0)
	System.out.println("got resp element in Assign PSP "+x);
	...
	if(vec.size()>3)   {
	  ...
	  if(MonitoringUtils.debug>0)
	    System.out.println("In Assign PSP id  as string is  :"+id);
	  try  {
	    Long lg=new Long (id.trim());
	    unique_id=lg.longValue();
	     if(MonitoringUtils.debug>0)
	       System.out.println(" In Assign PS id  in long  is  :"+unique_id);
	  ...
	  if(MonitoringUtils.debug>0)
	    System.out.println("In Assign PSP Sensor is  :"+Sensor);
	  Analyzer=(MonitoringUtils.parseString(vec.elementAt(3).toString(),'=')).elementAt(1).toString();
	  if(MonitoringUtils.debug>0)
	    System.out.println(" In Assign PSP analyzer is :"+Analyzer);
	}
	else  {
	...
	if (clusterID != null) {
      */
      out.print("<a href=\"$"+encodedClusterID+"/Search.PSP\">");
      // link to cluster
      out.print("Search </a> ");
      for(int i=0;i<responsetasks.size();i++)  {
	if(MonitoringUtils.debug>0)
	  System.out.println(" In Assign PSP vec element at ::"+i+responsetasks.elementAt(i).toString());
      }
