...
public class OrgAllocatorPlugIn extends  SimplePlugIn
{
    ...
    **/
	...
	{
		if(MonitoringUtils.debug>0)
			System.out.println("all send task is not empty");
	...
	{
		if(MonitoringUtils.debug>0)
			System.out.println("all update capabilities is not empty");
	...
	{
		if(MonitoringUtils.debug>0)
			System.out.println("all query task is not empty");
				...
				else
				{
					...
					if(MonitoringUtils.debug>0)
					{
						...
						for(;keys.hasMoreElements();)
						{
							...
							System.out.println("key after adding is :"+gotkey);
						}
			...
			if(pp!=null)	
			{
				...
				if(rtable.routertable.containsKey(sensordata.type))
				{
					...
					for(int i=0;i<orgcontainer.size();i++)
					{
						...
						{
							if(MonitoringUtils.debug>0)
								System.out.println("Going to do allocation for sensor data to org::::::::::::::"+destorg.toString());
    ...
    private void  process_SendTask(Iterator sendtask)
    {
		...
		if(MonitoringUtils.debug>0)
            	...
            	{
			if(MonitoringUtils.debug>0)
                		System.out.println("Got superior :"+org.toString());
    ...
    private void process_QueryTask(Enumeration query)
    {
       for(Enumeration e=query;e.hasMoreElements();)
       {
		...
		if(MonitoringUtils.debug>0)
			System.out.println("Got task in allocator for Query :::::: "+ task.toString());
            Organization org=findCapableOrg("YellowPageService");
            if(org!=null)
           {
		if(MonitoringUtils.debug>0)
               		System.out.println("Got capable org :"+org.toString());
    ...
    */
    ...
    {
        if(MonitoringUtils.debug>0)
		System.out.println("in process updateYp:::::::::::::");
	Organization org=findCapableOrg("YellowPageService");
        for(Enumeration e=capabilities;e.hasMoreElements();)
        {
		Task task=(Task)e.nextElement();
		if(MonitoringUtils.debug>0)
            		System.out.println("Got task in allocator for capab  "+ task.toString());
    ...
    private void process_ReportingTask(Enumeration reportingtasklist)
    {
        for(;reportingtasklist.hasMoreElements();)
        {
           ...
           {
               System.out.println("Got wrong pp in allocator form analyzers manager");
           }
        }
    }
	...
	if(MonitoringUtils.debug>0)
	{
		...
		if(MonitoringUtils.debug>0)
		{
			...
			if(MonitoringUtils.debug>0)
			{
                    		System.out.println("Check for superior collection is empty:"+ superior.isEmpty());
                    		dump(superior,"From inside of iterator loop");
			}
				...
				if(MonitoringUtils.debug>0)
                            System.out.println("Superior is  "+org.toString());
                            return org;
    ...
    // find ourself first
		...
		if(MonitoringUtils.debug>0)
			...
			if(MonitoringUtils.debug>0)
			{
          			System.out.println("Created role for search::::::::::::"+roleobj.getName());
			...
			Collection orgCollection = schedule.getMatchingRelationships(roleobj);
          		...
          		{
             			 System.out.println("Got capable org as empty ::::::::::::::::::::for role ::::::::::::::"+role);
          		}
    	...
    	{
              System.out.println("found no org capable of role::"+role);
    	}
