...
public class JaasClient {
  ...
  /**
     ...
     another component in a specific access controller context. This provides
     several advantages: 1) A policy may be associated with a particular
     instance of a component. 2) Instance information can be retrieved by
     the Security Manager when the contained component does not comply with
     the policy.
     Here's an example of how the method should be called:

     try {
       obj = JaasClient.doAs(agentName, new java.security.PrivilegedExceptionAction() {
