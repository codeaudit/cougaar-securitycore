...
class GetStackTrace < SecurityStressFramework
  ...
  def initialize(run)
    ...
    Dir.mkdirs(@stackbasedir) if !File.exists?(@stackbasedir)
    Dir.mkdirs("#{@stackbasedir}/scripts") if !File.exists?("#{@stackbasedir}/scripts")
  end
  ...
  def getStack(nodename)
    Thread.fork {
      ...
      begin
        ...
        if (nodeInfo.isCurrentlyRetrievingStackTrace)
          saveAssertion "getStackTrace", "Waiting to get stack trace... Currently retrieving one"
          return
        end
      rescue => ex
        saveAssertion "getStackTrace", "Unable to get node Info for #{nodename}"
        return
      end
      begin
        nodeInfo.isCurrentlyRetrievingStackTrace = true
        @@stackTraceId += 1
        ...
        getStackTraceFromProcFileSystem(nodeInfo, myStackTraceId)
      ...
      nodeInfo.isCurrentlyRetrievingStackTrace = false
    }
  end
  ...
  def getStackTraceFromProcFileSystem(nodeinfo, myStackTraceId)
    ...
    begin
      logfile = "#{@stackbasedir}/stack-#{nodeInfo.node.name}-#{nodeInfo.java_pid}.#{myStackTraceId}.log"
      #tmplogfile = "/tmp/stack-#{pid}.#{myStackTraceId}.log"

      # Build the script that will retrieve the stack trace
      script = "#{@stackbasedir}/scripts/cmd-stack-#{pid}-#{myStackTraceId}.sh"
      ...
      command = "sudo sh #{script} #{pid} #{logfile}"
  ...
  def getJavaPid(nodename)
    ...
    if (nodeInfo.java_pid == nil)
      raise "Unable to get Java PID for #{nodename}"
    end
    if (nodeInfo.node_pid == nil)
      raise "Unable to get Node PID for #{nodename}"
    end
  ...
  class NodeProcessInfo
    attr_accessor :node_pid, :java_pid, :node, :isCurrentlyRetrievingStackTrace
    def to_s
