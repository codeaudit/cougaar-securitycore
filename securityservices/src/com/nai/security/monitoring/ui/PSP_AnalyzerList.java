...
public class PSP_AnalyzerList extends PSP_BaseAdapter implements PlanServiceProvider, UISubscriber
{
  ...
  public void execute( PrintStream out,HttpInput query_parameters,PlanServiceContext psc,PlanServiceUtilities psu ) throws Exception 
  {
	...
	System.out.println("URL is form Plan Service context in Analyzer list  :"+sburl.toString());
      try  {
  ...
  public void printLinksToAnalyzer(PrintStream out,Vector links,Vector Names)
  {
    ...
    out.print("<p>\n");
    if(MonitoringUtils.debug>0)
      System.out.println("size of links in Analyzer List PSP  are ::"+links.size()+"size of name are "+Names.size());
    int length=links.size();
    for(int i=0;i<length;i++) {
      out.print("<p>\n");
      if(MonitoringUtils.debug>0)
	System.out.println(" Link to analyzers  in Analyzer List PSP  are :"+(String)links.elementAt(i)+ "for name :"+(String)Names.elementAt(i));
      URL url=null;
      try {
	url=new URL((String)links.elementAt(i));
      }
      catch(Exception exp) {
	exp.printStackTrace();
      }
	...
	out.print("<a href=\""+path+"Threats.PSP\">");
      }
      else {
