...
def getStartupTime(fileName)
  ...
  aFile.close
  return getTimeFromEvent(firstLine)
end

def getTimeFromEvent(line)
  startuptime = nil
  message = nil
  line.scan(/^([0-9]*)-([0-9]*)-([0-9]*) ([0-9]*):([0-9]*):([0-9]*),([0-9]*) (.*)/) { |x|
    time = x[0]
...
$okMessages = [
  ...
  [2*$startupDelay, "Guard is already set"],
  ...
  # ############################################################### 
  # Warnings generated by Security stresses
  # ############################################################### 
  [$startupDelay, "Stress4a50.txt.jar cannot be trusted"],
  [$startupDelay, "Stress4a51.txt.jar cannot be trusted"],
  [$startupDelay, "Stress4a52.txt.jar cannot be trusted"],
  [$startupDelay, "SecureConfigFinder.readExceptionList"],
...
def getLongestTransient()
  delay = 0
  $okMessages.each do |x|
    if x[0] != nil && x[0] > delay then
      delay = x[0]
    end
  end  
  return delay
...
def getLogResultsFileName(transient)
  if ($logResultsFileName == nil) 
    $logResultsFileName = "log4j-issues-#{Time.new.strftime("%m-%d-%Y_%H:%M:%S")}"
  end
  if transient
    return $logResultsFileName + ".log";
  else
    return $logResultsFileName + "-notransient.log"
  end
end
...
def searchIssues(removeOkPattern)
  ...
  logResultsFileName = getLogResultsFileName(true)
...
#    command << " | tee -a #{logResultsFileName} 2>&1"
    command << " >> #{logResultsFileName} 2>&1"
    #puts command
    system(command)
  end
  return $startupTime
...
def removeTransientIssues()
  #puts "entering remove transient issues"
  File.open(getLogResultsFileName(false), "w") do |reducedLogs|
    # puts("++ Checking #{reducedLogs.path()}")
    IO.foreach(getLogResultsFileName(true)) do |log|
      match = nil
      # puts "Line = #{log}"
      if (!((match = log.scan(/^\+\+ Checking (.*)/)).empty?))
        # puts("first match")
        puts(log)
        reducedLogs.puts(log)
      elsif (!((match = log.scan(/^([0-9]*)\:(.*)/)).empty?)) then
        # puts("second match")
        lineno = match[0][0]
        message = match[0][1]
        # puts "lineno = #{lineno}, message = #{message}"
        time = getTimeFromEvent(message)
        #
        # time == nil when the line is not an initial log line (hopefully)
        #
        if (time != nil) then
          # puts("Doing conservative comparison")
          # puts(time)
          # puts($startupTime + getLongestTransient())
          # puts(time <=> ($startupTime + getLongestTransient()))
          if ((time <=> ($startupTime + getLongestTransient())) == -1) then
            msgok = false
            $okMessages.each do |x|
              if (x[0] != nil) then
                # puts("Doing tighter comparison")
                # puts(time)
                # puts($startupTime + x[0])
                # puts(time <=> ($startupTime + x[0]))
                if (((time <=> ($startupTime + x[0])) == -1) &&
                    (message.index(x[1]) != nil)) then
                  msgok = true
                  break
                end
              end
            ...
            if (!msgok) then 
              puts("#{lineno}:#{message}")
              reducedLogs.puts("#{lineno}:#{message}")
            end
          else
            puts("#{lineno}:#{message}")
            reducedLogs.puts("#{lineno}:#{message}")
...
def parseTimeRelatedEvents(fileName, startupDelay)
  ...
  aFile.each_line { |line|
    time = getTimeFromEvent(line)
    if ( (time <=> ($startupTime + getLongestTransient())) == -1)
