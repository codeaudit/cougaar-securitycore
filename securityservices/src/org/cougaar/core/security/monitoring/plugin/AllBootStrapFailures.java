...
implements UnaryPredicate, QueryClassificationProvider {
  private static final String[] CLASSIFICATIONS = {
    IdmefClassifications.SECURITY_MANAGER_EXCEPTION,IdmefClassifications.JAR_VERIFICATION_FAILURE
  };
  /**
      ...
      if (msg instanceof Alert) {
        ...
        if (c != null) {
          for (int i = 0; i < c.length; i++) {
            if ((c[i].getName().equals(IdmefClassifications.SECURITY_MANAGER_EXCEPTION ))||
                (c[i].getName().equals(IdmefClassifications.JAR_VERIFICATION_FAILURE ))) {
              return true;
   ...
   * that support SECURITY_MANAGER_EXCEPTION & JAR_VERIFICATION_FAILURE failures .
   */
