   ...
   /** Creates new PSP_Search */
    ...
    public void execute( PrintStream out,HttpInput query_parameters,PlanServiceContext psc,PlanServiceUtilities psu ) throws Exception
    {
		...
		if(MonitoringUtils.debug>0)
            		System.out.println("cluster id  string is :"+psc.getServerPlugInSupport().getClusterIDAsString());
		...
		for(int i=0;i<alllinks.size();i++)
		{
            ...
            else
            {
                long id=0;
		if(MonitoringUtils.debug>0)
                	System.out.println("Got ver to look for");
			...
			if(MonitoringUtils.debug>0)
                    		System.out.println("Vernab are :"+type);
				...
				if(MonitoringUtils.debug>0)
                                	System.out.println("response obj is ::::::"+obj.toString());
                            }
