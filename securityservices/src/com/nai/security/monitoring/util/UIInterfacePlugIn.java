...
public class UIInterfacePlugIn extends org.cougaar.core.plugin.SimplePlugIn
{
    ...
    private IncrementalSubscription allorganization,allquerypsptask,allresponseTask,allcmd;
  ...
  **/
    ...
    **/
    ...
    protected void processquery(Enumeration e)
    {
		...
		if(MonitoringUtils.debug>0)
            		System.out.println("Going to publish query task from psp :"+qtsk.Type);
            publishQuerytask(qu);
   ...
   {
	if(MonitoringUtils.debug>0)
       		System.out.println("got response task::::::::::::");
       for(;restask.hasMoreElements();)
      {

        Task task=(Task)restask.nextElement();		
	if(MonitoringUtils.debug>0)
        	System.out.println("Got response task :"+task.toString()); 
		...
		if(MonitoringUtils.debug>0)
                	System.out.println("published respponse obj@@@@@@@@@@@@@@@@@@");
                publishAdd(obj);
        }
        else
        {
            System.out.println("got response task without prp:"+task.toString());
        }
